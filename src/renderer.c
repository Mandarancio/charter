...
svg_plane compute_plane(chart * c)
{
    int p_h = y_margin;
    int p_w = x_margin;
    int w = c->width -  (c->y_axis.label != NULL ? 3*p_w : 2*p_w);
    int h = c->height - (c->x_axis.label != NULL ? 3*p_h : 2*p_h);
    svg_plane p;
    p.h = h;
    p.w = w;
    p.top = p_h;
    p.left = (c->y_axis.label != NULL ? 2*p_w : p_w); 
    p.x0 =  (c->y_axis.label != NULL ? 2*p_w : p_w);
    p.y0 = h+p_h;
...
void line_plot_to_svg(char* buffer, chart* c, plot* p, svg_plane plane, unsigned int index)
{
    ...
    sprintf(buffer, "%s\" style=\"fill:none; stroke:%s; stroke-width:2;\" />\n",
